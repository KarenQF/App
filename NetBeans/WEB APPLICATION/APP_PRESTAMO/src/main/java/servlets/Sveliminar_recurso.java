...
@WebServlet("/Sveliminar_recurso")
public class Sveliminar_recurso extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String idEquipoParam = request.getParameter("id_equipo");
        ...
        if (idEquipoParam == null || idEquipoParam.isEmpty()) {
            session.setAttribute("mensaje", "ID de equipo no proporcionado.");
        ...
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
            ...
            String sql = "DELETE FROM recursos_tecnologicos WHERE id_equipo = ?";
            
            // Se usa PreparedStatement para evitar inyección SQL
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setInt(1, idEquipo); // Asignar el valor del ID en la consulta
                ...
                if (filasAfectadas > 0) {
                    ...
                    session.setAttribute("mensaje", "✅ Recursos tecnológico eliminado correctamente.");
                } else {
                    // Si no se encontró el usuario con el ID proporcionado
                    session.setAttribute("mensaje", "⚠️ No se encontró el recursos tecnológico a eliminar.");
                }
            }
            ...
            e.printStackTrace();
            session.setAttribute("mensaje", "❌ Error al eliminar el recursos tecnológico: " + e.getMessage());
        }
    ...
    public String getServletInfo() {
        return "Servlet que elimina un recurso tecnológico del sistema de préstamo.";
    }
}
